...
def layer_norm_matvec_rope_append(
    ...
    post_ln = rms_norm(
        inp=globals.hidden_states.bfloat16(),
        weight=globals.attn_ln_weights[layer_idx].bfloat16(),
        eps=globals.rms_norm_eps,
    )

    # with open("tensor_rope_ln_1b_dump.txt", "w") as f:
    ...
    for block_idx in range(
        ...
        # with open("tensor_qkv_proj_weight_1b_dump.txt", "w") as f:
        #     f.write(str(globals.qkv_proj_weights[layer_idx][start:end].tolist()))

        out = matmul_output
        
        
        # with open("tensor_qkv_proj_consumer_1b_dump.txt", "a") as f:
        #     if (block_idx == 0):
        #         f.write("[")
        ...
        # with open("tensor_qkv_proj_1b_dump.txt", "a") as f:
        #     if (block_idx == 0):
        #         f.write("[")
        #     f.write(", ".join(map(str, out.tolist())))
        #     f.write(", ")
...
def partial_attention(globals: Globals, instruction: PartialAttention):
    ...
    assert op_barriers[instruction.attention_head_idx] == 4
    # assert op_barriers[globals.num_attention_heads + instruction.kv_head_idx] == 4
    assert op_barriers[globals.num_attention_heads + instruction.attention_head_idx // gqa_ratio] == 4
    # assert (
    #     op_barriers[
    ...
    # with open("tensor_attention_1b_dump.txt", "w") as f:
