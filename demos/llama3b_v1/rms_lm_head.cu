...
template <typename Config, typename Globals> struct rms_lm_head {
    ...
    struct pipeline_specifics {
        ...
        static __device__ inline void store(megakernel::state<Config> &s, const Globals &g,
            ...
            kittens::warp::sync();
            // if (block_idx == 0 && kittens::warp::laneid() == 0) {
            //     printf("block_idx %d, logits_smem_bf[0]=%f, logits_smem_bf[1]=%f, logits_smem_bf[2]=%f, logits_smem_bf[3]=%f, "
            //         "logits_smem_bf[4]=%f, logits_smem_bf[5]=%f, logits_smem_bf[6]=%f, logits_smem_bf[7]=%f, "
            //         "logits_smem_bf[8]=%f, logits_smem_bf[9]=%f, logits_smem_bf[10]=%f, logits_smem_bf[11]=%f, "
            //         "logits_smem_bf[12]=%f, logits_smem_bf[13]=%f, logits_smem_bf[14]=%f, logits_smem_bf[15]=%f\n",
            //         block_idx,
            //         __bfloat162float(logits_smem_bf.data[0]), __bfloat162float(logits_smem_bf.data[1]), __bfloat162float(logits_smem_bf.data[2]), __bfloat162float(logits_smem_bf.data[3]),
            //         __bfloat162float(logits_smem_bf.data[4]), __bfloat162float(logits_smem_bf.data[5]), __bfloat162float(logits_smem_bf.data[6]), __bfloat162float(logits_smem_bf.data[7]),
            //         __bfloat162float(logits_smem_bf.data[8]), __bfloat162float(logits_smem_bf.data[9]), __bfloat162float(logits_smem_bf.data[10]), __bfloat162float(logits_smem_bf.data[11]),
            //         __bfloat162float(logits_smem_bf.data[12]), __bfloat162float(logits_smem_bf.data[13]), __bfloat162float(logits_smem_bf.data[14]), __bfloat162float(logits_smem_bf.data[15])
            //         );
