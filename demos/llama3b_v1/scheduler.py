...
from megakernels.demos.llama3b_v1.instructions import (
    DownProjResidual,
    Globals,
    Instruction,
...
def schedule_downproj(globs: Globals, layer_idx: int):
    ...
    num_col_splits = globs.intermediate_size // globs.hidden_size + 1
    sm_count = globs.sm_count()
...
def make_dag_layer(
    ...
    for attention_head_idx in range(globs.num_attention_heads):
        for partial_idx in range(num_attention_partitions):
            ins = PartialAttention(
                layer_idx=layer_idx,
                attention_head_idx=attention_head_idx,
            ...
            gqa_ratio = assert_div(globs.num_attention_heads, globs.num_kv_heads)
            k_start_dim = (
                globs.num_attention_heads + attention_head_idx // gqa_ratio
            ) * globs.head_dim
            v_start_dim = (
                globs.num_attention_heads
                + globs.num_kv_heads
                + attention_head_idx // gqa_ratio
            ) * globs.head_dim
...
class LLAMA3BV1ScheduleBuilder(ScheduleBuilder):
    @classmethod
