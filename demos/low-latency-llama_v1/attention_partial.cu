    ...
    struct launcher {
        static __device__ void wait_for_kv(const globals &g, megakernel::state<config> &s,
            ...
            while (*(volatile int *)&g.Bar[{
                       ...
                       LLAMA_1B_NUM_ATTENTION_HEADS + inst.attention_head_idx / GQA_RATIO}] < 4) {
                ...
                *(volatile int *)&g
                     .Bar[{inst.layer_idx, OPCODE_RMS_QKV_MatVecRopeAppend - 1,
                           LLAMA_1B_NUM_ATTENTION_HEADS +
                               LLAMA_1B_NUM_KV_HEADS + inst.attention_head_idx / GQA_RATIO}] < 4) {
                __nanosleep(config::GMEM_SPIN_LOOP_SLEEP_NANOS);
            }
    ...
    struct storer {
        ...
        store_o_no_skip(const globals &g, megakernel::state<config> &s,
            ...
            if (laneid == 0) {
                ...
                // }
                kittens::tma::store_async<cache_policy::EVICT_LAST>(
                    g.attn_out_intermediates, O_smem[0],
                    {0, q_head_start_idx, inst.partial_idx,
                     0});
            }
        }
        ...
        static __device__ void run(const globals &g, megakernel::state<config> &s) {
            ...
            int q_head_start_idx =
                inst.attention_head_idx; // 0, 1, 2, ..., 31
            ...
            if (laneid == 0) {
                // s.record(123 + laneid);
                // for (int i = 0; i < LLAMA_1B_HEAD_DIM; i++)
                //     printf("g.attn_out[%d] = %f\n", q_head_start_idx * LLAMA_1B_HEAD_DIM + i,
                //            __bfloat162float(g.attn_out[q_head_start_idx * LLAMA_1B_HEAD_DIM + i]));
                finish_QOL_page(s);
            }
