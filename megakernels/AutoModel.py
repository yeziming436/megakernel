...
from transformers.models.llama.modeling_llama import (
    ...
    apply_rotary_pos_emb as apply_llama_rotary_pos_emb,
)
...
ConfigType = Union[Qwen2Config, LlamaConfig]


class RMSNorm(nn.Module):
    def __init__(self, config: ConfigType):
        """
        Taken from AutoRMSNorm.
        """
        super().__init__()
        self.config = config
...
class AutoAttention(nn.Module):
    ...
    def __init__(
        self, config: ConfigType, extra_config: ExtraModelConfig, layer_idx: int
    ):
        super().__init__()
        self.config = config
        ...
        self.q_proj = nn.Linear(
            ...
            bias=self.extra_config.en_bias_q,
        )
        self.k_proj = nn.Linear(
            self.config.hidden_size,
            self.num_kv_heads * head_dim,
            bias=self.extra_config.en_bias_k,
        )
        self.v_proj = nn.Linear(
            self.config.hidden_size,
            self.num_kv_heads * head_dim,
            bias=self.extra_config.en_bias_v,
        )
        self.o_proj = nn.Linear(
            self.num_attention_heads * head_dim,
            config.hidden_size,
            bias=self.extra_config.en_bias_o,
        )
    ...
    def forward(
        ...
        if self.extra_config.interleave_rope:
            ...
            if self.extra_config.model_type.lower() == "llama":
                rope_fn = apply_llama_rotary_pos_emb
            elif self.extra_config.model_type.lower() == "qwen":
                rope_fn = apply_qwen2_rotary_pos_emb
            else:
                raise ValueError(
                    f"Unsupported model type for rotary embeddings: {self.extra_config.model_type}"
                )
...
class AutoMLP(nn.Module):
    def __init__(
        ...
        self.up_proj = nn.Linear(
            ...
            bias=self.extra_config.en_bias_up,
        )
        self.gate_proj = nn.Linear(
            self.config.hidden_size,
            self.intermediate_size,
            bias=self.extra_config.en_bias_gate,
        )
        self.down_proj = nn.Linear(
            self.intermediate_size,
            config.hidden_size,
            bias=self.extra_config.en_bias_down,
        )
...
class AutoBlock(nn.Module):
    def __init__(
        ...
        self.self_attn = AutoAttention(config, extra_config, layer_idx)
        self.mlp = AutoMLP(config, extra_config, layer_idx)
...
class AutoLMHead(nn.Module):
    def __init__(self, config: ConfigType, extra_config: ExtraModelConfig):
        super().__init__()
        self.config = config
        self.extra_config = extra_config
...
class AutoEmbeddings(nn.Module):
    def __init__(self, config: ConfigType):
        super().__init__()
        self.config = config
...
class AutoModel(nn.Module):
    ...
    def __init__(
        ...
        self.config = config
        self.extra_config = extra_config
        self.embed_tokens = AutoEmbeddings(config)

        layers = []
        for i in range(config.num_hidden_layers):
            layers.append(AutoBlock(config, extra_config, i))
        ...
        elif self.extra_config.model_type.lower() == "qwen2":
    ...
    def interleave_rope(self):
        ...
        for n in range(self.config.num_attention_heads):
            offset = n * (self.config.hidden_size // self.config.num_attention_heads)
        ...
        one_head_indices = indices_for_q[
            : (self.config.hidden_size // self.config.num_attention_heads)
        ]
...
class AutoForCausalLM(nn.Module):
    def __init__(
        self,
        config: ConfigType,
        extra_config: ExtraModelConfig,
        ...
        self.model = AutoModel(config, extra_config)

        self.lm_head = AutoLMHead(config, extra_config)
    ...
    def setup_caches(self):
        ...
        for layer_idx in range(self.config.num_hidden_layers):
            layer: AutoBlock = self.model.layers[layer_idx]  # type: ignore
    ...
    def from_pretrained(
        ...
        config: AutoConfig = AutoConfig.from_pretrained(model_name_or_path)  # type: ignore
    ...
    def stack_params(self):
        ...
        layers: list[AutoBlock] = self.model.layers  # type: ignore
        ...
        for self_attn in self_attns:
            ...
            if (
                self.extra_config.en_bias_q
                or self.extra_config.en_bias_k
                or self.extra_config.en_bias_v
            ):
                cat_bias = torch.cat(
                    [
        ...
        for i, self_attn in enumerate(self_attns):
            ...
            if stacked_qkv_biases is not None:
