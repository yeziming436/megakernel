...
from megakernels.demos.llama3b.instructions import (
    AttentionReduction,
    DownProjResidual,
    Globals,
...
def o_proj_residual(globals: Globals, instruction: O_ProjResidual):
    ...
    # with open("tensor_o_proj_dump.txt", "w") as f:
    #     f.write(str(globals.hidden_states.tolist()))
    # Barrier update
    ...
    # if instruction.end_block_idx == 192:
    #     with open("tensor_o_proj_dump.txt", "w") as f:
    #         f.write(str(globals.hidden_states.tolist()))
...
def layer_norm_matvec_rope_append(
    ...
    # with open("tensor_rope_ln_3b_dump.txt", "w") as f:
    #     f.write(str(post_ln.tolist()))
    ...
    for block_idx in range(
        ...
        # with open("tensor_qkv_proj_weight_3b_dump.txt", "w") as f:
        #     f.write(str(globals.qkv_proj_weights[layer_idx][start:end]))
        out = matmul_output

        # with open("tensor_qkv_proj_consumer_3b_dump.txt", "a") as f:
        #     if (block_idx == 0):
        #         f.write("[")
        ...
        # with open("tensor_qkv_proj_3b_dump.txt", "a") as f:
        #     if (block_idx == 0):
        #         f.write("[")
        #     f.write(", ".join(map(str, out.tolist())))
        #     f.write(", ")
    ...
    # if instruction.end_output_block_idx == 320:
    #     with open("tensor_rope_dump.txt", "w") as f:
    #         f.write(str(globals.post_ln_rope_q.tolist()))
    #     with open("tensor_k_cache_dump.txt", "w") as f:
    #         f.write(str(globals.k_cache[layer_idx, :, pos_id].view(-1).tolist()))
    #     with open("tensor_v_cache_dump.txt", "w") as f:
    #         f.write(str(globals.v_cache[layer_idx, :, pos_id].view(-1).tolist()))
...
def partial_attention(globals: Globals, instruction: PartialAttention):
    ...
    assert op_barriers[instruction.attention_head_idx] == 8
    # assert op_barriers[globals.num_attention_heads + instruction.kv_head_idx] == 8
    assert op_barriers[globals.num_attention_heads + instruction.attention_head_idx // gqa_ratio] == 8
    # assert (
    #     op_barriers[
    ...
    assert (
        op_barriers[
            globals.num_attention_heads + globals.num_kv_heads + instruction.attention_head_idx // gqa_ratio
        ]
        == 8
    )
    # with open("tensor_rope_dump.txt", "w") as f:
    ...
    layer_idx = instruction.layer_idx
    attention_head_idx = instruction.attention_head_idx
    kv_head_idx = attention_head_idx // gqa_ratio
    ...
    # if (layer_idx == 0 and kv_head_idx == 0):
    #     with open("tensor_attention_3b_dump.txt", "w") as f:
    #         f.write(str(out.tolist()))
    # elif (layer_idx == 0): 
    #     with open("tensor_attention_3b_dump.txt", "a") as f:
    #         f.write("," + str(out.tolist()))
    ...
    # if instruction.kv_head_idx == 7:
    #     with open("tensor_attention_out_dump.txt", "w") as f:
    #         f.write(str(globals.attn_out.tolist()))
    #     with open("tensor_attention_lse_dump.txt", "w") as f:
    #         f.write(str(globals.attn_lse_intermediates.tolist()))
