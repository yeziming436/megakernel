...
from megakernels.demos.qwen1_5b.instructions import (
    AttentionReduction,
    DownProjResidual,
    Globals,
...
def down_proj_residual(globals: Globals, instruction: DownProjResidual):
    # Barrier check
    op_barriers = globals.barriers[instruction.layer_idx, instruction.prev_opcode() - 1]
    assert op_barriers[0] == 560  # 8960 / 16
...
def layer_norm_matvec_rope_append(
    ...
    for block_idx in range(
        ...
        out = matmul_output + globals.qkv_proj_biases[layer_idx][start:end]
...
def partial_attention(globals: Globals, instruction: PartialAttention):
    ...
    for i in range(gqa_ratio):
        assert op_barriers[instruction.kv_head_idx * gqa_ratio + i] == 16
    assert op_barriers[globals.num_attention_heads + instruction.kv_head_idx] == 16
    assert (
        op_barriers[
