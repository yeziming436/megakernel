...
from megakernels.instructions_moe import BaseGlobals, Instruction


@dataclass
class Globals(BaseGlobals):
    # new para
    expert_router_logits: Tensor
    expert_router_weights: Tensor
    expert_mask: Tensor
    expert_hit: Tensor
...
class Router(MatVecAdd):
    ...
    def cost(self, globs: Globals):
        return (
            (self.end_block_idx - self.start_block_idx)
            * globs.expert_gate_block_size
            * globs.num_experts
        )


@dataclass
class Dispatch_expert(Instruction):
    ...
    @classmethod
    def prev_opcode(cls) -> int:
        return Router.opcode()

    # mark_maybe_wrong cost should be calculated
    def cost(self, globs: Globals):
        return (
            (self.end_block_idx - self.start_block_idx)
            * globs.expert_gate_block_size
