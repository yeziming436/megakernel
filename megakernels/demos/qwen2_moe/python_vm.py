...
from megakernels.AutoModel import (
...
def matvec_with_weight_residual(
    mat: Tensor,
    vec: Tensor,
    record: Tensor,
    residual: Tensor,
    ...
    for block_idx in range(start_block_idx, end_block_idx):
        ...
        matvec_out = matvec_out * weight.to(matvec_out.dtype) + record[start:end]
        residual[start:end] += matvec_out.to(residual.dtype)
...
def down_proj_residual(globals: Globals, instruction: DownProjResidual):
    ...
    # mark_maybe_wrong
    matvec_with_weight_residual(
        mat=globals.expert_down_weights[instruction.layer_idx][instruction.expert_idx],
        vec=globals.silu_out,
        record=globals.hidden_states,
        residual=globals.final_hidden_states,
        weight=globals.expert_router_weights[instruction.layer_idx][instruction.top_x][
            instruction.idx
        ],
...
def layer_norm_double_matvec_silu(
    ...
    for block_idx in instruction.block_idxs:
        ...
        up_matvec, start, end = matvec(
            # mark_maybe_wrong expert_weights 的权重是什么形状
            mat=globals.expert_up_weights[instruction.layer_idx][
                instruction.expert_idx
            ],
            vec=post_ln[instruction.top_x],
            block_size=block_size,
            block_idx=block_idx,
        )

        gate_matvec, _, _ = matvec(
            mat=globals.expert_gate_weights[instruction.layer_idx][
                instruction.expert_idx
            ],
            vec=post_ln[instruction.top_x],
            block_size=block_size,
        ...
        globals.silu_out[start:end].index_add_(
            0, instruction.top_x, post_silu.to(globals.hidden_states.dtype)
        )
...
def layer_norm_matvec_rope_append(
    ...
    post_ln = rms_norm(
        # mark_maybe_wrong
        inp=globals.final_hidden_states,
...
def router(globals: Globals, instruction: Router):
    ...
    block_size = globals.expert_gate_block_size

    # mark_maybe_wrong
    expert_matvec, start, end = matvec(
        mat=globals.expert_router_gate_weights[instruction.layer_idx],
        vec=post_ln,
        block_size=block_size,
        block_idx=instruction.start_block_idx,
    )

    globals.expert_router_logits[start:end] = expert_matvec


def dispatch_expert(globals: Globals, instruction: Dispatch_expert):
    op_barriers = globals.barriers[instruction.layer_idx, instruction.prev_opcode() - 1]
    assert op_barriers[0] == 128

    router_weights = F.softmax(globals.expert_router_logits, dim=-1)
    router_weights, selected_experts = torch.topk(
        router_weights, globals.num_experts_per_token, dim=-1
    )

    if globals.norm_topk_prob:
        routing_weights /= routing_weights.sum(dim=-1, keepdim=True)

    globals.expert_mask = torch.nn.functional.one_hot(
        selected_experts, num_classes=globals.num_experts
    ).permute(2, 1, 0)

    globals.expert_hit = torch.greater(
        globals.expert_mask.sum(dim=(-1, -2)), 0
    ).nonzero()
...
INSTRUCTION_TO_SOLVER = {
    ...
    Router: router,
    Dispatch_expert: dispatch_expert,
}
