...
class PyTorchGenerator(Generator):
    ...
    def generate(
        ...
        for i in range(ntok):
            ...
            " Record generated token ID "
            output_tokenids.append(decode_output.output_ids.item())

            output_pos = input_token_pos + 1
            print(f"Generated token ID: {decode_output.output_ids.item()}")
            output_tokens[:, output_pos] = decode_output.output_ids.squeeze(-1)

        return output_tokenids
...
class MK_Generator(Generator):
    ...
    def generate(
        ...
        for i in range(ntok):
            ...
            output_tokenids.append(output_ids.item())

            print(f"Generated token ID: {output_ids.item()}")
